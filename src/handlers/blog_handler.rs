//! 博客相关 API 路由及其处理器。
//!
//! 所有路由挂载在 `/blog` 下，由 `routes.rs` 中的顶层路由统一注册。
...
/// `GET /blog/childTypes/{parent}` — 获取某个父分类的子分类。
...
/// `GET /blog/posts/type/{type_key}` — 按分类获取文章（含子分类）。
...
/// `GET /blog/article?article_name=xxx` — 获取文章 Markdown 内容。
...
/// `POST /blog/upload` — 上传 Markdown 文章。
///
